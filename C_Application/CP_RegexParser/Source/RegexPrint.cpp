...
{
    std::string l_indent(p_depth * 3, ' ');
    p_ostream << l_indent << "-> [" << p_regexNode.m_id << "]";

    if (p_regexNode.m_nodeType == Leaf)
    {
        const auto& l_leafNode = static_cast<const LeafNode&>(p_regexNode);
        if (l_leafNode.m_leafNodeType == LetterNode)
        {
            p_ostream  << " ('" << l_leafNode.m_letter << "')";
        }
        else 
        {
            p_ostream << " ( . )";
    ...
    {
        const auto& l_operatorNode = static_cast<const OperatorNode&>(p_regexNode);
        p_ostream << " (" << l_operatorNode.m_operatorType << ")";
    }

    p_ostream << "\n";

    for (const auto& l_child : p_regexNode.m_childNodes)
    {
        if (l_child)
        {
            printRegexNodeRecursive(p_ostream, *l_child, p_depth + 1);
        }
        else
        {
            p_ostream << l_indent << "   -> [NULLPTR]\n";
        }
    }
}
